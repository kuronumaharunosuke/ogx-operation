// seed.js — DBへシード投入。CLI（node --experimental-sqlite seed.js --reset）でも、
// サーバ起動時の自動シード（runSeed）でも使える。
const db = require("./db");
...
function runSeed(reset = false) {
  ...
  if (count > 0 && !reset) return { skipped: true };
  ...
  return {
    eps: db.prepare("SELECT COUNT(*) c FROM eps").get().c,
    tasks: db.prepare("SELECT COUNT(*) c FROM op_template").get().c,
    toi: db.prepare("SELECT COUNT(*) c FROM toi_field").get().c,
  };
}
...
if (require.main === module) {
  const r = runSeed(process.argv.includes("--reset"));
